/**
 * @file move_obstacle_case1.cpp
 * @author Bonafini Marco
 * @brief ROS node that moves case1 obstacles when signaled by algoritmo_prevenzione_collisioni.cpp
...
void publish_model_pose(ros::NodeHandle n, std::vector<double>& obj_pos) {
  ...
  model_msg.model_name = "v_obs";
  model_msg.pose.position.x = -obj_pos[0];
  model_msg.pose.position.y = 0.3;
  model_msg.pose.position.z = 1.2;
  ...
  //std::cout << "PUB MODEL POSE " << obj_pos[0] << std::endl;
...
void add_collision_obj(){
  ...
  remove_collision_objects("v_obs");
    
  // ADD VERTICAL OBSTACLE   -------------------------------------------------------------

  moveit_msgs::CollisionObject collision_object;
  collision_object.header.frame_id = move_group_arm->getPlanningFrame();
  collision_object.id = "v_obs";
...
void wait_for_message(ros::NodeHandle n){
    ...
    obj_pos[0] = -0.14; obj_pos[1] = 0.25; obj_pos[2] = 0.6; obj_pos[3] = 0.1; obj_pos[4] = 0.1; obj_pos[5] = 0.74;
    ...
    while(move) {
        ...
        obj_pos[0] += 0.00014;
        move_collision_objects(n, obj_pos, "v_obs");
        //obj_pos[0] += 0.004; if gazebo obstacle motion
...
int main(int argc,char** args){
    ...
    ros::init(argc, args, "move_obstacle1_node");
    ros::NodeHandle n;
